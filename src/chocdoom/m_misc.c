...
int _FIO_GetFileSize64(char *pFileName, size64_t *pSize);

long M_FileLength(char* pFileName)
{
  size64_t size;
  size.lo = 0;
  size.hi = 0;

  _FIO_GetFileSize64(pFileName, &size);
  uart_printf("M_FileLength: %d", size.lo);
