...
static wad_file_t *W_StdC_OpenFile(char *path)
{
    ...
    result->wad.length = M_FileLength(path);
    result->fstream = fstream;

    return &result->wad;
#else
    char pPathWithDrive[128];
    snprintf(pPathWithDrive, sizeof(pPathWithDrive), "B:/%s", path);

    stdc_wad_file_t *result;
    FILE* file;
    file = FIO_OpenFile(pPathWithDrive, O_RDONLY | O_SYNC);
    if (!file)
    {
	...
	result->wad.file_class = &stdc_wad_file;
	result->wad.mapped = NULL;
	result->wad.length = M_FileLength(pPathWithDrive);
	result->fstream = file;
    uart_printf("E\n");
	return &result->wad;
#endif
